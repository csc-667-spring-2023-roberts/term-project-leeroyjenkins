...
router.get("/:gameID", async (req, res) => {
  ...
  try {
    ...
    if (statusInfo.length > 0) {
      ...
      community = [
        statusInfo[0].community.slice(1, 3),
        statusInfo[0].community.slice(4, 6),
        statusInfo[0].community.slice(7, 9),
        statusInfo[0].community.slice(10, 12),
        statusInfo[0].community.slice(13, 15),
      ];
...
router.post("/:gameID", async (req, res) => {
  ...
  try {
    ...
    if (check.length === 0) {
      let { name, minimum, maximum, count, players, plimit, dealer } =
        await game_table.getData(gameID);
      if (count < plimit) {
        count += 1;
        ...
        io.to(`game-${gameID}`).emit(socketCalls.SYSTEM_MESSAGE_RECEIVED, {
          message,
          gameID,
          timestamp: Date.now(),
        });
        io.to(`game-${gameID}`).emit(socketCalls.PLAYER_JOINED_RECEIVED, {
          username,
        });
...
router.post("/:gameID/create", async (req, res) => {
  ...
  try {
    ...
    await game_status.createStatus(
      gameID,
      parseInt(0),
      parseInt(table.minimum / 2),
      communityCards,
      playerCards,
      playerChips,
      table.players,
    ...
    io.to(`game-${gameID}-${smallBlindSeat}`).emit(socketCalls.UPDATE_CHIPS, {
      chips: table.minimum / 2,
    });
    io.to(`game-${gameID}-${smallBlindSeat}`).emit(
      socketCalls.GAME_DEAL_CARDS,
      { cards: playerCards[table.dealer] }
    );
    io.to(`game-${gameID}-${bigBlindSeat}`).emit(
      socketCalls.ACTION_PLAYERS_TURN,
      { callAmount: table.minimum, bigBlind: true }
    );
    io.to(`game-${gameID}`).emit(socketCalls.UPDATE_POT, {
      pot: table.minimum / 2,
    });
  } catch (err) {
...
router.post("/:gameID/bet", async (req, res) => {
  ...
  try {
    ...
    if (parseInt(bet) < wallet.wallet) {
      ...
      io.to(`game-${gameID}-${x + 1}`).emit(socketCalls.UPDATE_CHIPS, {
        chips: chips[x],
      });
      // check: next player
      ...
      while (index !== x) {
        ...
        if (
          (currentBet !== -1 && currentBet < recentBet) ||
          currentBet === -2
        ) {
          ...
          proceed = false;
          io.to(`game-${gameID}-${index + 1}`).emit(
            socketCalls.ACTION_PLAYERS_TURN,
            { callAmount: diff, bigBlind: false }
          );
          res.status(200).json({ message: "Success" });
          break;
        }
      ...
      if (chips[bigBlindIndex] >= info.minimum) {
        io.to(`game-${gameID}-${bigBlindIndex + 1}`).emit(
          socketCalls.GAME_DEAL_CARDS,
          { cards: g[0].player_cards[bigBlindIndex] }
        );
        res.status(200).json({ message: "Success" });
      }
      // all players called
      if (proceed) {
        ...
        if (round === 1) {
          io.to(`game-${gameID}`).emit(socketCalls.GAME_FLOP, {
            cards: [
              g[0].community.slice(1, 3),
              g[0].community.slice(4, 6),
              g[0].community.slice(7, 9),
            ],
          });
          io.to(`game-${gameID}-${nextSeat}`).emit(
            socketCalls.ACTION_PLAYERS_TURN,
            { callAmount: 0, bigBlind: false }
          );
          res.status(200).json({ message: "Success" });
        } else if (round === 2) {
          io.to(`game-${gameID}`).emit(socketCalls.GAME_TURN_RIVER, {
            card: g[0].community.slice(10, 12),
          });
          io.to(`game-${gameID}-${nextSeat}`).emit(
            socketCalls.ACTION_PLAYERS_TURN,
            { callAmount: 0, bigBlind: false }
          ...
          io.to(`game-${gameID}`).emit(socketCalls.GAME_TURN_RIVER, {
            card: g[0].community.slice(13, 15),
          });
          io.to(`game-${gameID}-${nextSeat}`).emit(
            socketCalls.ACTION_PLAYERS_TURN,
            { callAmount: 0, bigBlind: false }
  ...
  try {
    ...
    for (let i = 0; i < allSeats.length; i++) {
      if (allSeats[i].seat > playerSeat) {
        await player_table.updateSeat(
          gameID,
          allSeats[i].player_id,
          allSeats[i].seat - 1
        );
      }
    }
    ...
    io.to(`game-${gameID}`).emit(socketCalls.PLAYER_LEFT_RECEIVED, {
      username,
    });
